...
@test "fish_user_paths_add: it tell me added" (fish_user_paths_add $TEMP_DIR > /dev/null) $status -eq 0

@test "fish_user_paths_add: really added" (contains -- $TEMP_DIR $fish_user_paths) $status -eq 0

@test "fish_user_paths_add: avoid add multiple times" (fish_user_paths_add $TEMP_DIR > /dev/null) $status -eq 2

@test "fish_user_paths_add: still have me" (contains -- $TEMP_DIR $fish_user_paths) $status -eq 0

@test "path not exist" ! -d $NON_EXIST_PATH
@test "fish_user_paths_add: add non-exist path" (fish_user_paths_add $NON_EXIST_PATH > /dev/null) $status -eq 1
...
@test "fish_user_paths_rm: removed" (fish_user_paths_rm $TEMP_DIR > /dev/null) $status -eq 0
